...
export function user(app, options, done) {
	...
	app.post(
		'/user/register',
		{ schema: { body: UserEntity, tags: ['user'] } },
		async (req, res) => {
	...
	app.get('/users', { schema: { tags: ['user'] } }, async (req, res) => {
		const result = await usersCollection.find(req.body || {}).toArray();
		res.send(result);
	});

	app.get('/profile', { schema: { tags: ['user'] } }, async (req, res) => {
		if (!req.user) {
	...
	app.get(
		'/user/:id',
		{ schema: { params: UserId, tags: ['user'] } },
		async (req, res) => {
	...
	app.put(
		'/user/:id',
		{ schema: { params: UserId, body: UserEntity, tags: ['user'] } },
		async (req, res) => {
			const result = await usersCollection.updateOne(
